...
class MetaNeRF(MetaModule):
    """
    Standard NeRF (Fourier xyz) with a NeRF-style skip (concat enc_xyz at chosen layers).
    - xyz: FrequencyEncoder(pe_dim=pe_dim_xyz, include_input=include_input_xyz)
    - trunk: depth 'sigma_depth' with 'skips' (e.g., (4,)), concat enc_xyz before those layers
    - heads: sigma_head (TruncExp), geo_head (G)
    - color: MetaSequential depth=color_depth on [geo_feat, dir_enc]
    - dir: SH(4) or Frequency(4, include_input=True)
    """
    ...
    def __init__(
        ...
        *,
        hidden: int = 256,
        sigma_depth: int = 8,
        skips: Tuple[int, ...] = (4,),  # original NeRF-style skip at layer 4
        geo_feat_dim: int = 15,
        ...
        include_input_dir: bool = True,  # applies if encoding_dir="frequency"
    ) -> None:
        super().__init__()
        ...
        # -----------------------
        # Geometry trunk with NeRF skip concat
        # -----------------------
        self.trunk = nn.ModuleList()
        for i in range(self.sigma_depth):
            # if we are at a skip layer, the input is [hidden || enc_xyz]
    ...
    def density(
        ...
        """xyz → trunk (with NeRF skip concat) → {'sigma': (...,1), 'geo_feat': (...,G)}."""
        enc = self._enc_xyz(x)
        h = enc
        for i, blk in enumerate(self.trunk):
            if i in self.skips and i != 0:
                h = torch.cat([h, enc], dim=-1)  # NeRF-style skip concat
    ...
    def forward(
        ...
        """Full forward: returns {'sigma': (...,1), 'rgb': (...,3)}."""
