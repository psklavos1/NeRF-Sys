#!/usr/bin/env python3
"""
Mega-NeRF Dataset Statistics (DRB-consistent)
=============================================

Logs detailed statistics for a Mega-NeRF–style dataset and reports all
poses, rotations, and translations in the canonical **DRB world frame**
(Down, Right, Back).
...
def parse_args():
    p = ArgumentParser(description="Log Mega-NeRF dataset statistics in DRB coords.")
...
# ---------------------------------------------------------------
# Simple dual logger (stdout + file)
# ---------------------------------------------------------------
...
def _signed_roll_deg(R_stack: torch.Tensor) -> torch.Tensor:
    ...
    world_up = torch.tensor([-1., 0., 0.], dtype=R_stack.dtype, device=R_stack.device)  # DRB Up = -Down(+X)
...
def log_dataset_info(ds_root: Path) -> None:
    ...
    logger.write(f"images (train/val)  : {len(t_meta)} / {len(v_meta)} (total={len(all_meta)})")
    ...
    if alt_band is not None:
        logger.write(f"altitude_range_enu (m): [{float(alt_band[0]):.3f}, {float(alt_band[1]):.3f}]")
    ...
    def collect(paths, ray_count):
        for mp in paths:
            ...
            intr_fx.append(fx); intr_fy.append(fy)
            intr_cx.append(cx); intr_cy.append(cy)
            xdeg, ydeg = _fov_deg(W, H, fx, fy)
            fovx.append(xdeg); fovy.append(ydeg)
    ...
    if cams_drb.numel() == 0:
        ...
        for label, v in zip(("Down(+X)", "Right(+Y)", "Back(+Z)"), (cams_drb[:, 0], cams_drb[:, 1], cams_drb[:, 2])):
            logger.write(f"{label}: {_range_mean_std(v)}")
            logger.write(f"{label} pct {pct}: {[round(x,6) for x in _quantiles(v,pct)]}")
        out_of_range = (cams_drb.abs() > 1.0001).any(dim=1).sum().item()
        if out_of_range:
            logger.write(f"⚠ WARNING: {out_of_range} camera translations fall outside [-1,1] after normalization.")
    ...
    if cams_world.numel() == 0:
        ...
        for label, v in zip(("Down(+X)", "Right(+Y)", "Back(+Z)"), (Xw, Yw, Zw)):
            ...
            logger.write(f"{label} pct(m) {pct}: {[round(x,3) for x in _quantiles(v,pct)]}")
        ...
        if cams_world.shape[0] > 1:
            ...
            if d.numel() > 0:
                ...
                logger.write(f"Baseline distance among cameras (m): min={d.min():.3f}, "
    ...
    if R_list_drb:
        ...
        ortho_err = torch.norm(torch.transpose(R_stack, 1, 2) @ R_stack - torch.eye(3), dim=(1, 2))
        logger.write(f"det(R_drb): mean={dets.mean().item():.6f}, min={dets.min().item():.6f}, max={dets.max().item():.6f}")
        logger.write(f"orthogonality ||R^T R - I||_F: mean={ortho_err.mean().item():.2e}, "
        ...
        ex = torch.tensor([1., 0., 0.], dtype=forwards.dtype)  # Down(+X)
        ey = torch.tensor([0., 1., 0.], dtype=forwards.dtype)  # Right(+Y)
        ez = torch.tensor([0., 0., 1.], dtype=forwards.dtype)  # Back(+Z)
    ...
    if intr_fx:
        ...
        try:
            ...
            logger.write(f"Principal point offset (mean abs): "
