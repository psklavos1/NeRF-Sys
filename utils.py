...
def save_inner_lr(path, inner_lr):
    torch.save({"version": 1, "value": float(inner_lr)}, path)


def load_inner_lr(path, device, learnable=False):
    if not Path(path).exists():
        return None
    obj = torch.load(path, map_location="cpu")
    val = float(obj.get("value", obj))
    return (
        nn.Parameter(torch.tensor(val, device=device), requires_grad=learnable)
        if learnable
        else val
    )
...
# used to smaple a batch of episodes
def get_meta_batch(P, task_data):
    if P.data_type == "img":
        return get_image_data(P, task_data)
    elif P.data_type == "video":
        return task_data["videos"].size(0), task_data["videos"]
    elif P.data_type == "ray":
        return get_support_query_rays(
            task_data, P.support_split, per_task_shuffle=True, seed=P.seed
        )
    else:
        raise NotImplementedError()


def get_image_data(P, task_data):
    """
    Splits each image in the batch into a support or query set for meta-learning.
    No spatial region slicing is applied; each task is a full image.

    Args:
        P: Config object with:
            - data_ratio (float): Fraction of images to use as support
        task_data (Dict): Contains:
            - 'img': Tensor of shape [B, C, H, W]

    Returns:
        dict containing the support/query batch sizes and contexts in a tuple

    """
    images = task_data["imgs"]  # [B, C, H, W]
    B = images.size(0)
    num_tasks = B  # ? Not sure yet

    return num_tasks, images


class rsvrBase(ABC):
    """
    Logic for Continual Learnign Replay approaches
    Args:
...
# * ---------------------------------- New helpers --------------------------------------------------
def get_support_query_rays(
    group: dict,
    ratio: float,
    per_task_shuffle: bool = True,
    seed: int | None = None,
):
    """Split per-cell batched tensors along the ray dim while keeping grouping.

    Args:
        group: {cid: {"rays": (B,N,8), "rgbs": (B,N,3), "image_id": (B,)?}}
        ratio: support fraction in (0,1)
        per_task_shuffle: True => independent split per task; False => shared split per cell
        seed: if not None, use a local RNG seeded to make splits reproducible

    Returns:
        {"support": {cid: {...}}, "query": {cid: {...}}}
    """
    assert 0.0 < ratio < 1.0
    sup_by_cell, qry_by_cell = {}, {}

    # infer a device to place the RNG on (match tensors)
    any_cell = next(iter(group.values()))
    dev = any_cell["rays"].device

    # local generator (optional, reproducible)
    g = None
    if seed is not None:
        g = torch.Generator(device=dev)
        g.manual_seed(int(seed))

    for cid, b in group.items():
        rays, rgbs = b["rays"], b["rgbs"]  # (B,N,8), (B,N,3)
        B, N, C8 = rays.shape
        assert C8 == 8 and rgbs.shape == (
            B,
            N,
            3,
        ), "Expected rays:(B,N,8), rgbs:(B,N,3)"

        sN = max(1, min(int(round(ratio * N)), N - 1))

        if per_task_shuffle:
            # Independent split per task (fast & better for learning)
            perms = torch.rand(B, N, generator=g, device=dev).argsort(dim=1)  # (B,N)
            sidx, qidx = perms[:, :sN], perms[:, sN:]

            sidx8 = sidx.unsqueeze(-1).expand(-1, -1, 8)
            qidx8 = qidx.unsqueeze(-1).expand(-1, -1, 8)
            sidx3 = sidx.unsqueeze(-1).expand(-1, -1, 3)
            qidx3 = qidx.unsqueeze(-1).expand(-1, -1, 3)

            rays_sup = torch.gather(rays, 1, sidx8)
            rays_qry = torch.gather(rays, 1, qidx8)
            rgbs_sup = torch.gather(rgbs, 1, sidx3)
            rgbs_qry = torch.gather(rgbs, 1, qidx3)
        else:
            # Same split for all tasks in this cell
            perm = torch.randperm(N, generator=g, device=dev)
            sidx, qidx = perm[:sN], perm[sN:]
            rays_sup, rgbs_sup = rays[:, sidx], rgbs[:, sidx]
            rays_qry, rgbs_qry = rays[:, qidx], rgbs[:, qidx]
    ...
    return {"support": sup_by_cell, "query": qry_by_cell}
...
def collate_episodes_batched(batch):
    """
    Collate a list of B episodes (each:
      {"support":{"rays":[S,8],"rgbs":[S,3],"image_ids":[V]},
       "query":  {"rays":[Q,8],"rgbs":[Q,3],"image_ids":[V]},
       "cell_id": (optional) Long[]})
    into:
      {"support":{"rays":[B,S,8],"rgbs":[B,S,3],"image_ids":[B,V]},
       "query":  {"rays":[B,Q,8],"rgbs":[B,Q,3],"image_ids":[B,V]},
       "cell_id":[B] (if present)}
    """
    # sanity: all episodes in the batch should agree on V
    V_sup = [ep["support"]["image_ids"].numel() for ep in batch]
    V_qry = [ep["query"]["image_ids"].numel() for ep in batch]
    if len(set(V_sup)) != 1 or len(set(V_qry)) != 1:
        raise ValueError(
            f"collate_episodes: image_ids lengths differ in batch: "
            f"support={V_sup}, query={V_qry}. Make sure max_views_per_episode is consistent."
        )

    def pack(side):
        rays = torch.stack([ep[side]["rays"] for ep in batch], 0)  # [B,S|Q,8]
        rgbs = torch.stack([ep[side]["rgbs"] for ep in batch], 0)  # [B,S|Q,3]
        views = torch.stack([ep[side]["image_ids"] for ep in batch], 0)  # [B,V]
        return {"rays": rays, "rgbs": rgbs, "image_ids": views}

    out = {"support": pack("support"), "query": pack("query")}
    if "cell_id" in batch[0]:
        out["cell_id"] = torch.stack([ep["cell_id"] for ep in batch], 0)  # [B]
    return out
